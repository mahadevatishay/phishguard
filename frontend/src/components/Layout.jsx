    ...
    <div className="flex h-screen bg-gray-950 overflow-hidden">
      ...
      `}>
        ...
        <div className="h-14 flex items-center justify-between px-3 border-b border-gray-800/60">
          ...
          {collapsed && (
            <div className="w-7 h-7 bg-cyan-500 rounded-lg flex items-center justify-center mx-auto">
              <Shield className="w-4 h-4 text-gray-950" />
            </div>
          )}
          <button onClick={() => setCollapsed(!collapsed)}
            className="hidden lg:flex p-1 rounded-md hover:bg-gray-800 text-gray-500 hover:text-gray-300 transition-colors">
          ...
          {!collapsed ? (
            <div className="flex items-center gap-2 p-2 rounded-lg">
              ...
              <button onClick={() => { logout(); navigate('/login') }}
                className="p-1 hover:text-red-400 text-gray-500 transition-colors" title="Logout">
                <LogOut className="w-3.5 h-3.5" />
              </button>
            </div>
          ) : (
            <button onClick={() => { logout(); navigate('/login') }}
              className="w-full flex justify-center p-2 hover:text-red-400 text-gray-500" title="Logout">
